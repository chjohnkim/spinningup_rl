...
class HindsightExperienceReplayWrapper:
    '''
    Wrapper around replay buffer in order to use HER.
    This implemenation is inspired by the one found in 
    https://stable-baselines.readthedocs.io/en/master/modules/her.html
    ...
    def store(self, obs, act, rew, next_obs, done, next_achieved_goal):
        # Update current episode buffer, NOT replay buffer
        self.episode_transitions.append(
            (obs, act, rew, next_obs, done, next_achieved_goal)
        )
    ...
    def sample_her_transitions(self):
        # Add transitions (and imagined ones) to buffer only when episode ends
        self._store_episode()
    ...
    def _store_episode(self):
        ...
        '''
        # For each transition in episode, create a set of artificial transitions
        for transition_idx, transition in enumerate(self.episode_transitions):
            obs, act, rew, next_obs, done, next_achieved_goal = transition
            ...
            artificial_goals = [
                self._sample_achieved_goal(self.episode_transitions, transition_idx)
            ...
            for artificial_goal in artificial_goals:
                ...
                obs, act, rew, next_obs, done, next_achieved_goal = copy.deepcopy(transition)

                # Update the desired goal in the transition
                obs[-self.goal_dim:] = artificial_goal
                next_obs[-self.goal_dim:] = artificial_goal

                # Update the reward according to the new desired goal
                rew = self.env.compute_reward(next_achieved_goal, artificial_goal, info=None)
                done = False
