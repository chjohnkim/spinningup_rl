...
class DDPG(Agent):
    def __init__(self, 
        ...
        self._update_every = self.cfg["update_every"] # JOHN
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        :type timesteps: int
        """
        if timestep >= self._learning_starts and timestep % self._update_every == 0: # JOHN
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                ...
                target_values = sampled_rewards + self._discount_factor * target_q_values # JOHN
                clip_return = 1/(1 - self._discount_factor) # JOHN
                target_values = torch.clamp(target_values, -clip_return, 0) # JOHN
            # compute critic loss
            ...
            policy_loss += actions.pow(2).mean() # JOHN
        ...
        self.target_policy.update_parameters(self.policy, polyak=self._polyak) # JOHN
        self.target_critic.update_parameters(self.critic, polyak=self._polyak) # JOHN
